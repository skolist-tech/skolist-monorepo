...
async def insert_batch_to_supabase(
    ...
    """
    Optimized batch insertion for questions and related data.
    Reduces round trips to Supabase by batching multiple inserts.
    """
    questions = await try_retry_batch(batch, batch_idx, ctx, max_retries)
    if not questions:
        return 0

    logger.info(f"Started Batch Inserting {len(questions)} questions in supabase")
    ...
    for item in questions:
        ...
        # Pre-generate UUID to allow batching related tables
        ...
        try:
            ...
            logger.error(f"Failed to prepare batch payload for question: {e}")
            continue
    ...
    try:
        ...
        # 2. Batch insert initial versions
        ...
        # 4. Batch insert concept maps (unique only)
        if concept_maps_to_insert:
            ...
            for m in concept_maps_to_insert:
                ...
                if key not in seen_maps:
                    unique_concept_maps.append(m)
                    seen_maps.add(key)

            if unique_concept_maps:
                await supabase_client.table("gen_questions_concepts_maps").insert(unique_concept_maps).execute()
