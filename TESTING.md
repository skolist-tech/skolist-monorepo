...
- **Write an e2e test for every user-experienceable action** — a small control, a larger flow, or a complete journey. If a person can see it or do it in the app, add a Playwright spec under `e2e/tests/` that performs that action. Specs that create their own paper use `workerPair()` (`e2e-teacher-N` / `e2e-student-N`). Specs that sit the seeded JEE/NEET papers keep Teacher 1/2 and Student 1/2/3 and import `test` from `shared-seed.ts`.
