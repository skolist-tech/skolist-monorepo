...
-- Get online test by share code (for students joining via link)
...
BEGIN
    ...
    -- Get test details
    SELECT ot.*, d.paper_title, d.paper_subtitle, d.institute_name, d.maximum_marks
    INTO v_test
    ...
    -- Check if user is in same org (students) or is teacher
    ...
    -- Return test info (without questions for now, those will be fetched separately when starting attempt)
    RETURN jsonb_build_object(
        'id', v_test.id,
        'title', v_test.title,
...
COMMENT ON FUNCTION "public"."get_online_test_by_share_code" IS 'Gets online test details by share code. Validates user has access.';
