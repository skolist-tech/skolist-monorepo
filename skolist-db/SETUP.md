...
Seeded users also get a public avatar SVG in the `seed_assets` storage bucket (`public.users.avatar_url`). A few assessment questions get figure SVGs in the same bucket (`assessment.questions.image_url`) and the same markup in `svg_image_code` (plus option badge SVGs on JEE Main Mock Test 1 physics Q1). Re-run `_002_seed_users.py` / `_004_seed_assessment.py` (or `python seed.py`) after `supabase db reset` so those objects exist.
