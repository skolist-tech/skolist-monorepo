# Assessments

Assessments is a **standalone exam product** for tuitions, schools, and coaching centres that need students to sit timed tests on a computer. It is **not** a deep piece of the main Skolist strategy loop (diagnose gaps → personalised improvement). It shares Skolist’s accounts and database, but it is a separate product with its own sign-in screen: email, password, and organisation code only — no sign-up, Google, or phone login.

It is built so the attempt feels familiar to anyone who has used an official computer-based exam — instructions first, countdown clock, question palette, save vs mark-for-review, then a result.

## Who it is for

- **Coaching institutes, tuitions, and schools** that run mock or internal tests.
- **Students** practising or taking assigned papers under exam-like conditions.
- **Teachers** (or academic staff) who author a paper, publish it, assign it to students, and later review attempts and scores.

## What a student can do

- See papers that have been **assigned** to them.
- Read **instructions** and agree to exam rules before the clock starts.
- Work through **sections** (Physics, Chemistry, Maths, and similar), jump around, and use a **palette** that shows visited / answered / marked-for-review the way NTA interfaces do.
- Answer multiple-choice and numerical questions, clear a response, and submit when finished (or when time runs out).
- Open a **result**: marks and a look back at what they put.

The goal is not a casual quiz. It is a fair, focused attempt that matches how high-stakes exams actually feel, so practice transfers.

## What a teacher can do

- Create and edit a paper (structure, questions, assignment).
- Publish it so students can start.
- Review a student’s attempt after the fact.

## What it is not

This is not Skolist’s marketing site, and it is not QGen’s day-to-day “build me a worksheet” studio. It does not tutor the student after the result. It is the **exam hall**: sign in, sit the paper, see the score.

## How it relates to other Skolist apps

Same accounts and API as the rest of the platform, different job. QGen is for composing papers inside the Skolist teacher workflow. Assessments is for institutes that just need a serious test sitting. Do not assume a change here should look or feel like QGen.
